...
> Answer: The mean of the group is equal to a standard value
...
> We found that Ozone levels are belwo the 50 ppm threshold using a one sided 
...
```{r}
#Exercise 2:
Ozone.plot2 <- ggplot(EPAair, aes(x = Ozone, fill=as.factor(Year))) +
...
Ozone.plot.Box <- ggplot(EPAair, aes(x = as.factor(Year), y= Ozone))  +
  geom_boxplot()

print(Ozone.plot.Box)
```
...
EPA.regression <- lm(EPAair$PM2.5 ~ EPAair$Ozone)
...
EPA.Scatter <- ggplot(EPAair, aes(x = Ozone, y = PM2.5, color = Year)) + 
  geom_point(alpha=0.2) + ylim(0,75) + geom_smooth( method = lm , se = FALSE, color = 'blue')
